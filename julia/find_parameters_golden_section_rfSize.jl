...
for windowType in [0 1]
    ...
    for rfOverlap in 0.2:0.1:0.9
        ...
        for k in 1:50
            ...
            temp_size = []
            temp_y1 = round(Int, y_1,)
            temp_y2 = round(Int, y_2,)
            if !(temp_y1 in keys(f_size))
                push!(temp_size, temp_y1)
            end 
            if !(temp_y2 in keys(f_size))
                push!(temp_size, temp_y2)
            end 
            for rfSize in temp_size
                println("rfSize = ", rfSize)
                ...
                alltimes[key] =  tm
                f_size[rfSize] = 0.6*g + 0.4*t 
            end
            println("\n\n", f_size[temp_y1])
            println("\n\n", f_size[temp_y2])

            if f_size[temp_y1] < f_size[temp_y2]
                y_r = y_2
                y_2 = y_1
                y_1 = y_r - (y_r - y_l)/phi
            else
