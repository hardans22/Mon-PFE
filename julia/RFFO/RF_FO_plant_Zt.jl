...
function buildM(instance_dict,rf_or_fo)
    ...
    model[:x] = @variable(model, 0 <= x[i in P, k in T, t in k:len_T])
    ...
    model[:c4] = @constraint(model, c4[t in T], sum(x[i,t,j] for i in P, j in t:len_T) - u[t] <= c[t])
    ...
    #objective
    #=
    obj =  sum(sum(holding_cost[i,j] for j in k:t-1) * x[i,k,t] for i in P, k in T, t in k:len_T) + sum(set_up_cost[i,t]*y[i,t] for i in P, t in T) + dot(mtn_cost, z) 
    coef = sum(mtn_cost)/(len_T)
    obj += sum(coef*u[t] for t in T)
    =#
    coef = sum(mtn_cost)/(len_T)
    @objective(model, Min, sum(sum(holding_cost[i,j] for j in k:t-1) * x[i,k,t] for i in P, k in T, t in k:len_T) + sum(variable_prod_cost[i,k]*x[i,k,t] for i in P, k in T, t in k:len_T) + sum(set_up_cost[i,t]*y[i,t] for i in P, t in T) + sum(mtn_cost[t]*z[t] for t in T) + sum(coef*u[t] for t in T))
...
function solve_model(model, w_fix, w_mip, sy, sz, rf_or_fo)
    ...
    return Dict("obj" => obj, "sx" => sx, "sy" => sy, "sz" => sz, "sc" => sc, "su" => su, "model" => model)
end
...
function RelaxAndFix(mdl, rfSize, step, instance_dict)
    ...
    while true
        ...
        if all(isinteger, sy) && all(isinteger, sz)
            return Dict("obj" => obj, "sx" => sx, "sy" => sy, "sz" => sz, "sc" => sc, "su" => su)
        end
    end
end
