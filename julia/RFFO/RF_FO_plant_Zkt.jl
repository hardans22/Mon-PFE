...
function buildM(instance_dict,rf_or_fo)
    ...
    @objective(model, Min, sum(sum(holding_cost[i,j] for j in k:t-1) * x[i,k,t] for i in P, k in T, t in k:len_T) + sum(variable_prod_cost[i,k]*x[i,k,t] for i in P, k in T, t in k:len_T) + sum(set_up_cost[i,t]*y[i,t] for i in P, t in T) + sum(mtn_cost[t]*z[t,t] for t in T) + sum(coef*u[t] for t in T))
